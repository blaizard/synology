...
if __name__ == '__main__':
	...
	command = ["ssh", "-nNT", str(config["auth"])]
	...
	regexpr = r"ssh\s+-nNT\s+" + re.escape(config["auth"]) + r"\b"
	runningList = []
	...
	if len(runningList) > 0:
		...
		openPortList = shell(["ssh", str(config["auth"]), "netstat", "-an"])
		regexpr = r"tcp.*:\b" + re.escape(str(config["ports"]).split(":")[0]) + r"\b.*LISTEN"
